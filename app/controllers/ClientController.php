...
class ClientController extends BaseController
{
    ...
    public function search_tax_address()
    {
        ...
        if (Request::ajax()) {
            if ($field_type == "I") {
                $data = TaxOfficeAddress::where("parent_id", "==", "0")->select("office_id",
                    "office_name")->get();
            } elseif ($field_type == "C") {
                $data = CorporationTaxOffice::select("corp_tax_id as office_id", "office_name")->
                    get();
            }
        }
    ...
    public function archive_client()
    {
        ...
        foreach ($clients_id as $client_id) {
            if ($status == "Archive") {
                ...
                Client::where('client_id', '=', $client_id)->update(array("is_archive" => "N",
                        "show_archive" => "N"));
            }
    ...
    public function show_archive_client()
    {
        ...
        $affected = Client::whereIn("user_id", $groupUserId)->where("show_archive", "=",
    ...
    public function delete_relationship()
    {
        ...
        $client_type = Input::get("client_type");
        $resp = ClientRelationship::where("client_relationship_id", "=", $delete_index)->
            delete();
        echo $resp;
    }
    ...
    public function save_officers_into_relation()
    {
        ...
        if (isset($officers->items[$key]) && count($officers->items[$key]) > 0) {
            ...
            $data['client_id'] = $client_id;
            Client::where("client_id", "=", $client_id)->update(array("is_relation_add" =>
                    "Y", 'type' => 'non'));
            //$data['base_url'] = url();
    ...
    public function indonboardsnotes()
    {
        ...
        if ($getclientid['onboardingnotes_id']) {
            Indonboardingnote::where("onboardingnotes_id", "=", $getclientid->
                onboardingnotes_id)->update($data);
        } else {
