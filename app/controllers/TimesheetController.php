    ...
    public function time_sheet_reports($type)
    {
		...
		if(!empty($time_sheet_report)){
				foreach($time_sheet_report as $key=>$val){
					...
					$data2[$key]['client_detail'] = StepsFieldsClient::query()->select(DB::raw('SELECT * FROM `steps_fields_clients` WHERE `client_id` = '.$val['rel_client_id'].' AND (`field_name` = "business_name"
OR `field_name` = "client_name")'));
